...
import com.runtai.shoppingcart.adapter.ExpandableListAdapter;
...
public class MainActivity extends Activity implements UpdateView, View.OnClickListener {

    private ExpandableListView mExpandableListView;
    private SmoothCheckBox mCbSelectAll; //全选
    private TextView mTvAllMoney; //价格
    private Button mBtnBuy; //结算
    ...
    public void onClick(View view) {
        switch(view.getId()){
            case R.id.cb_SelectAll: //全选
                selectAll();
                break;
            case R.id.btn_Settlement: //结算按钮
    ...
    private void selectAll() {
        ...
        if (!mCbSelectAll.isChecked()) {//全选选中操作
            ...
            for (int i = 0; i < goodBean.getContent().size(); i++) {
                ...
                for (int n = 0; n < goodBean.getContent().get(i).getGooddetail().size(); n++) {
                    if (!goodBean.getContent().get(i).getGooddetail().get(n).isselected()) {
                        int count = Integer.valueOf(goodBean.getContent().get(i).getGooddetail().get(n).getCount());
                        int price = Integer.valueOf(goodBean.getContent().get(i).getGooddetail().get(n).getPrice());
                        allCount += count;
